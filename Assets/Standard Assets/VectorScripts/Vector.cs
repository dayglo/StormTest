...
public class Vector {
	...
	static bool WrongArrayLength (VectorLine line, int arrayLength, FunctionName functionName) {
		...
		if (line.continuous) {
			if (arrayLength != pointsLength-1) {
				LogError(functionNames[(int)functionName] + " array for \"" + line.name + "\" must be length of points array minus one for a continuous line (one entry per line segment)");
				return true;
			}
		}
		else if (arrayLength != pointsLength/2) {
			LogError(functionNames[(int)functionName] + " array in \"" + line.name + "\" must be exactly half the length of points array for a discrete line (one entry per line segment)");
			return true;
		}
		return false;
	}
	...
	private static bool CheckArrayLength (VectorLine line, FunctionName functionName, int segments, int index) {
		...
		if (line.isPoints) {
			if (index + segments > linePoints) {
				if (index == 0) {
					LogError("Vector: " + functionNames[(int)functionName] + ": The number of segments cannot exceed the number of points in the array for \"" + line.name + "\"");
					return false;
				}
				LogError("Vector: Calling " + functionNames[(int)functionName] + " with an index of " + index + " would exceed the length of the Vector array for \"" + line.name + "\"");
				return false;				
			}
			return true;
		}
		...
		if (line.continuous) {
			if (index + (segments+1) > linePoints) {
				if (index == 0) {
					LogError("Vector: " + functionNames[(int)functionName] + ": The length of the array for continuous lines needs to be at least the number of segments plus one for \"" + line.name + "\"");
					return false;
				}
				LogError("Vector: Calling " + functionNames[(int)functionName] + " with an index of " + index + " would exceed the length of the Vector array for \"" + line.name + "\"");
				return false;
			...
			if (index + segments*2 > linePoints) {
				if (index == 0) {
					LogError("Vector: " + functionNames[(int)functionName] + ": The length of the array for discrete lines needs to be at least twice the number of segments for \"" + line.name + "\"");
					return false;
				}
				LogError("Vector: Calling " + functionNames[(int)functionName] + " with an index of " + index + " would exceed the length of the Vector array for \"" + line.name + "\"");
				return false;
	...
	public static void SetColors (VectorLine line, Color[] lineColors) {
		...
		if (!line.isPoints) {
			...
			LogError("Vector: SetColors: Length of lineColors array in \"" + line.name + "\" must be same length as points array");
			return;
		}
	...
	static void SetStartAndEnd (VectorLine line, ref int start, ref int end) {
		start = (line.minDrawIndex == 0)? 0 : (line.continuous)? line.minDrawIndex : line.minDrawIndex/2;
	...
	public static void SetWidths (VectorLine line, int[] lineWidths) {
		if (lineWidths == null) {
		...
		if (line.isPoints) {
			if (lineWidths.Length != GetPointsLength (line)) {
				LogError("Vector: SetWidths: line widths array must be the same length as the points array for \"" + line.name + "\"");
				return;
			}
		}
		...
		for (int i = 0; i < end; i++) {
			line.lineWidths[i] = (float)lineWidths[i] * .5f;
		}
	}
	...
	public static void DrawLine (VectorLine line, Transform thisTransform) {
		...
		if (!cam) {
			...
			if (!cam) {	// If that didn't work (no camera tagged "Main Camera")
				LogError("Vector.DrawLine: You must call SetCamera before calling DrawLine for \"" + line.name + "\"");
				return;
			}
		}
		if (line.smoothWidth && line.lineWidths.Length == 1 && GetPointsLength (line) > 2) {
			LogError("Vector: DrawLine called with smooth line widths for \"" + line.name + "\", but Vector.SetWidths has not been used");
			return;
		}
	...
	public static void DrawLine3D (VectorLine line, Transform thisTransform) {
		...
		if (!cam3D) {
			SetCamera3D();
			if (!cam3D) {
				LogError("Vector.DrawLine3D: You must call SetCamera or SetCamera3D before calling DrawLine3D for \"" + line.name + "\"");
				return;
			}
		}
		if (line.points3 == null) {
			LogError("Vector: DrawLine3D can only be used with a Vector3 array, which \"" + line.name + "\" doesn't have");
			return;
		}
		if (line.smoothWidth && line.lineWidths.Length == 1 && line.points3.Length > 2) {
			LogError("Vector: DrawLine3D called with smooth line widths for \"" + line.name + "\", but Vector.SetWidths has not been used");
			return;
		}
	...
	public static void DrawLineViewport (VectorLine line) {
		DrawLineViewport(line, null);
	}
		
	public static void DrawLineViewport (VectorLine line, Transform thisTransform) {
		if (error || !line.active) return;
		if (line == null || line.vectorObject == null) {
			LogError("Vector.DrawLineViewport: the line must not be null");
			return;
		}
		if (!cam) {
			SetCamera();
			if (!cam) {	// If that didn't work (no camera tagged "Main Camera")
				LogError("Vector.DrawLineViewport: You must call SetCamera before calling DrawLineViewport for \"" + line.name + "\"");
				return;
			}
		}
		if (line.points2 == null) {
			LogError("Vector: DrawLineViewport can only be used with a Vector2 array, which \"" + line.name + "\" doesn't have");
			return;
		}
		if (line.smoothWidth && line.lineWidths.Length == 1 && GetPointsLength (line) > 2) {
			LogError("Vector: DrawLineViewport called with smooth line widths for \"" + line.name + "\", but Vector.SetWidths has not been used");
			return;
		}
		...
		int end = (line.maxDrawIndex == 0)? line.points2.Length-1 : line.maxDrawIndex;
	...
	public static void SetTextureScale (VectorLine line, Transform thisTransform, float textureScale, float offset) {
		int pointsLength = GetPointsLength (line);
		int end = line.continuous? pointsLength-1 : pointsLength;
		int add = line.continuous? 1 : 2;
		int idx = 0;
		int widthIdx = 0;
	...
	public static void DrawPoints3D (VectorPoints line, Transform thisTransform) {
		...
		if (!cam3D) {
			SetCamera3D();
			if (!cam3D) {
				LogError("Vector.DrawPoints3D: You must call SetCamera or SetCamera3D before calling DrawPoints3D for \"" + line.name + "\"");
				return;
			}
		}
		if (line.points3 == null) {
			LogError("Vector: DrawPoints3D can only be used with a Vector3 array, which \"" + line.name + "\" doesn't have");
			return;
		}
	...
	public static void DestroyLine (ref VectorPoints line) {
		if (line != null) {
			MonoBehaviour.Destroy (line.vectorObject);
			MonoBehaviour.Destroy (line.mesh);
			MonoBehaviour.Destroy (line.meshFilter);
	...
	public static void DestroyObject (ref VectorPoints line, GameObject go) {
		DestroyLine (ref line);
		if (go != null) {
			MonoBehaviour.Destroy (go);
		}
	...
	public static void MakeRectInLine (VectorLine line, Vector3 topLeft, Vector3 bottomRight, int index) {
		...
		if (line.continuous) {
			if (index + 5 > linePoints) {
				if (index == 0) {
					LogError("Vector: MakeRectInLine: The length of the array for continuous lines needs to be at least 5 for \"" + line.name + "\"");
					return;
				}
				LogError("Vector: Calling MakeRectInLine with an index of " + index + " would exceed the length of the Vector2 array for \"" + line.name + "\"");
				return;
			}
			if (line.points2 == null) {
			...
			if (index + 8 > linePoints) {
				if (index == 0) {
					LogError("Vector: MakeRectInLine: The length of the array for discrete lines needs to be at least 8 for \"" + line.name + "\"");
					return;
				}
				LogError("Vector: Calling MakeRectInLine with an index of " + index + " would exceed the length of the Vector2 array for \"" + line.name + "\"");
				return;
	...
	private static void MakeSplineInLine (VectorLine line, Vector2[] splinePoints2, Vector3[] splinePoints3, int segments, int index, bool loop) {
		...
		if ( (line.continuous && pointCount < index + (segments+1)) || (!line.continuous && pointCount < index + (segments*2)) ) {
			if (line.points2 != null) {
	...
	public static void SetLineDistances (VectorLine line) {
		int pointsLength = GetPointsLength (line);
		if (line.distances == null || line.distances.Length != (line.continuous? pointsLength : pointsLength/2+1)) {
			line.distances = new float[line.continuous? pointsLength : pointsLength/2+1];
		}
		...
		if (line.points3 != null) {
			if (line.continuous) {
				for (int i = 0; i < pointsLength; i++) {
					Vector3 diff = line.points3[i] - line.points3[i+1];
					...
					line.distances[i+1] = (float)totalDistance;
				}
			}
			else {
				var count = 1;
				for (int i = 0; i < pointsLength; i += 2) {
					...
					line.distances[count++] = (float)totalDistance;
				}
			}
		}
		else {
			if (line.continuous) {
				for (int i = 0; i < pointsLength; i++) {
					Vector2 diff = line.points2[i] - line.points2[i+1];
	...
	public static float GetLineLength (VectorLine line) {
		int pointsLength = GetPointsLength (line);
		if (line.distances == null || line.distances.Length != (line.continuous? pointsLength : pointsLength/2+1)) {
			SetLineDistances (line);
		}
		return line.distances[line.distances.Length-1];
	}

	public static Vector2 GetLinePoint01 (VectorLine line, float distance) {
		return GetLinePoint (line, Mathf.Lerp(0.0f, GetLineLength (line), distance) );
	}

	public static Vector2 GetLinePoint (VectorLine line, float distance) {
		if (line.points2 == null) {
			LogError("Vector: GetLinePoint only works with Vector2 points");
			return Vector2.zero;
		}
		if (line.points2.Length < 2) {
			LogError("Vector: GetLinePoint needs at least 2 points in the points2 array");
			return Vector2.zero;
		}
		if (line.distances == null) {
			SetLineDistances (line);
		}
		int i = 1;
		int end = line.distances.Length-1;
		while (distance > line.distances[i] && i < end) {
			i++;
		}
		if (line.continuous) {
			return Vector2.Lerp(line.points2[i-1], line.points2[i], Mathf.InverseLerp(line.distances[i-1], line.distances[i], distance));
		}
		return Vector2.Lerp(line.points2[(i-1)*2], line.points2[(i-1)*2+1], Mathf.InverseLerp(line.distances[i-1], line.distances[i], distance));
	}

	public static Vector3 GetLinePoint3D01 (VectorLine line, float distance) {
		return GetLinePoint3D (line, Mathf.Lerp(0.0f, GetLineLength (line), distance) );
	}
	
	public static Vector3 GetLinePoint3D (VectorLine line, float distance) {
		if (line.points3 == null) {
			LogError("Vector: GetLinePoint3D only works with Vector3 points");
			return Vector3.zero;
		}
		if (line.points3.Length < 2) {
			LogError("Vector: GetLinePoint3D needs at least 2 points in the points3 array");
			return Vector3.zero;
		}
		if (line.distances == null) {
			SetLineDistances (line);
		...
		if (line.continuous) {
			return Vector3.Lerp(line.points3[i-1], line.points3[i], Mathf.InverseLerp(line.distances[i-1], line.distances[i], distance));			
		}
		return Vector3.Lerp(line.points3[(i-1)*2], line.points3[(i-1)*2+1], Mathf.InverseLerp(line.distances[i-1], line.distances[i], distance));
	}
	...
	public static void ZeroPointsInLine (VectorLine line, int index) {
		...
		if (index >= pointsLength) {
			LogError("Vector: index out of range for \"" + line.name + "\" when calling ZeroPointsInLine. Index: " + index + ", array length: " + pointsLength);
			return;
		}
